...
export default function LoginWrapper() {    
    ...
    const personas = [        
        ...
        { consonant: false, en: 'official', hi: 'अधिकारी', applicationId: process.env.NEXT_PUBLIC_FUSIONAUTH_STATE_APP_ID, redirectUrl: '/admin'},
        { consonant: false, en: 'operator', hi: 'संचालक', applicationId: process.env.NEXT_PUBLIC_FUSIONAUTH_STATE_APP_ID, redirectUrl: '/admin' },        
    ]  
    if(selectedPersona) {
        return <Layout>
    ...
    return (
        ...
        <div className={`${styles.grid} ${styles['grid-two']}`}>
            ...
            <div onClick={() => { setSelectedPersona(persona) }} key={index} className={`card ${index === (personas.length - 1) ? 'center' : ''}`}>
