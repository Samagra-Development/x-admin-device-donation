...
const sendSMS = async (msg, templateId, contactNumber) => {
  try {
    ...
    if (response.data?.error) {
      sendLog(
        `*✅ samarth-device*: SMS status update notification sending to *${contactNumber}* failed`
      );
      return { error: response.data?.error, success: null };
    } else if (response.data?.success) {
      sendLog(
        `*⚠️samarth-device*: SMS status update notification successfully sent to *${contactNumber}*`
      );
